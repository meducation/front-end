(function() {
  ...
  mediaFileFormControllerFunction = function($rootScope, $scope) {
    ...
    unbind = $rootScope.$on('externalfileurlchange', function(event, url) {
  ...
  mediaFileUploadControllerFunction = function($rootScope, $scope) {
    $scope.options = {
      dataType: 'xml',
      autoUpload: true
    };
    ...
    return $scope.$on('fileuploaddone', function(event, data) {
      ...
      return $rootScope.$emit('externalfileurlchange', externalFileUrl);
    });
  };
